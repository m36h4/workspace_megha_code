"""EC backbone: ECViT + adapter.

Ported from EdgeCrafter (Apache-2.0):
    https://github.com/Intellindust-AI-Lab/EdgeCrafter
    engine/edgecrafter/ecvit.py

The upstream module fuses last-N transformer-block outputs, reshapes back to
2D, and bilinearly resamples to three pyramid levels (strides 8/16/32) which
the HybridEncoder consumes. State-dict keys are kept byte-identical to upstream
so weight conversion needs no per-tensor remap inside the backbone.
"""
...
from ..dfine.encoder import ConvNormLayer_fuse
...
def _no_grad_trunc_normal_(tensor, mean, std, a, b):
    ...
    if (mean < a - 2 * std) or (mean > b + 2 * std):
        warnings.warn(
            "mean is more than 2 std from [a, b] in trunc_normal_. Distribution may be incorrect.",
            stacklevel=2,
        )
    with torch.no_grad():
...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        act_layer = nn.GELU  # upstream forces GELU regardless of arg
...
class ViTAdapter(nn.Module):
    """ECViT backbone wrapper: fuse last-N transformer outputs into a 3-level pyramid.

    Output: list of three tensors at strides 8, 16, 32 (relative to input).
    """
    ...
    def __init__(
        ...
        num_heads: int = 3,
        patch_size: int = 16,
        interaction_indexes: Tuple[int, ...] = (10, 11),
        ...
        **kwargs,
        ...
        if embed_layer not in _EMBED_LAYERS:
        ...
        if num_levels != 3:
            raise NotImplementedError("Only num_levels=3 supported")
        ...
        self.backbone = VisionTransformer(
            ...
            patch_size=patch_size,
            embed_layer=_EMBED_LAYERS[embed_layer],
            ffn_layer=_FFN_LAYERS[ffn_layer],
            ffn_ratio=ffn_ratio,
            **kwargs,
        )
    ...
    def forward(self, x):
        H_c = x.shape[2] // self.patch_size
        W_c = x.shape[3] // self.patch_size
        bs = x.shape[0]
