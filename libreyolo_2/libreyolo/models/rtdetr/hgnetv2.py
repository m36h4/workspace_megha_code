"""HGNetv2 backbone for RT-DETR (L/X variants).

Ported from the official RT-DETR implementation:
https://github.com/lyuwenyu/RT-DETR/blob/main/rtdetrv2_pytorch/src/nn/backbone/hgnetv2.py

Original copyright header preserved below.
"""
...
class HGNetv2(nn.Module):
    """HGNetv2 backbone (PP-HGNetV2) for RT-DETR L/X.

    Args:
        name: One of 'L', 'X', 'H'.
        use_lab: Whether to use LearnableAffineBlock in network.
        return_idx: Stage indices to return as multi-scale features.
        freeze_at: Freeze stem and the first `freeze_at` stages (-1 to disable).
        freeze_norm: Replace BatchNorm2d with FrozenBatchNorm2d.
        pretrained: Load PaddleClas SSLD ImageNet weights.
    """
    ...
    def __init__(
        ...
        self.out_channels = [self._out_channels[i] for i in return_idx]
        self.out_strides = [self._out_strides[i] for i in return_idx]
