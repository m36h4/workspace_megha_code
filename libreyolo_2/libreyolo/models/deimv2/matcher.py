"""Hungarian matcher for native DEIMv2 training.

Ported from DEIMv2 (https://github.com/Intellindust-AI-Lab/DEIMv2).
Copyright (c) 2025 The DEIMv2 Authors. All Rights Reserved.

Differences from upstream:
- ``@register()`` / ``__share__`` stripped.
- Uses LibreYOLO's already-flat DEIM box ops.
- Keeps DEIMv2's epoch-aware matcher switch.
"""
...
from ..deim.box_ops import box_cxcywh_to_xyxy, box_iou, generalized_box_iou
...
class HungarianMatcher(nn.Module):
    ...
    def forward(
        ...
        if self.change_matcher and epoch >= self.matcher_change_epoch:
            ...
            C = -class_score * torch.pow(bbox_iou, self.iou_order_alpha)
        else:
            if self.use_focal_loss:
