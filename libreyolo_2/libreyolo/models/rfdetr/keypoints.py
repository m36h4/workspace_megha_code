"""GroupPose-style keypoint helper primitives for RF-DETR.

Ported from RF-DETR (https://github.com/roboflow/rf-detr).
Copyright (c) 2025 Roboflow, Inc. All Rights Reserved.
Licensed under the Apache License, Version 2.0.

Keypoint predictions are dense ``[N, K, KEYPOINT_PRED_DIM]`` tensors with a fixed trailing
channel layout:
...
``7``           ``class``    Per-keypoint class-logit contribution added to detection logits.
==============  ===========  ===========================================================
"""
...
class ConditionalQueryInitializer(nn.Module):
    ...
    def forward(self, query_features: torch.Tensor) -> torch.Tensor:
        """Return modulated query embeddings of shape ``(B, num_queries, out_dim)``.
...
def compute_l1_keypoint_loss(
    ...
    """Compute per-matched-target keypoint losses.

    The Gaussian NLL follows the r-flow formulation ``0.5 * maha2 / area - (log_l11 + log_l22)``.
    It intentionally omits the Gaussian constant and does not clamp the precision Cholesky
    ...
    Returns:
        Tuple ``(location, findable BCE, visible BCE, raw Gaussian NLL)``, each shape ``(N,)``.
    """
...
def compute_keypoint_matching_cost(
    ...
    cost_l1 = torch.zeros(
        (b, num_queries, n_targets), device=all_pred_keypoints.device, dtype=all_pred_keypoints.dtype
    )
