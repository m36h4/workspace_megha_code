"""DEIMCriterion — MAL + L1 + GIoU + FGL + DDF, with GO-LSD match union.
...
class DEIMCriterion(nn.Module):
    ...
    def loss_labels_mal(self, outputs, targets, indices, num_boxes, values=None):
        """Modified Align Loss from DEIM's Dense O2O recipe."""
        assert "pred_boxes" in outputs
        idx = self._get_src_permutation_idx(indices)
        if values is None:
            src_boxes = outputs["pred_boxes"][idx]
    ...
    def forward(self, outputs, targets, **kwargs):
        ...
        if "aux_outputs" not in outputs:
            # DEIM always emits aux_outputs in training mode; absence
            # indicates a model construction bug or misuse during inference.
            raise RuntimeError(
                "DEIMCriterion.forward requires 'aux_outputs' in the model's "
