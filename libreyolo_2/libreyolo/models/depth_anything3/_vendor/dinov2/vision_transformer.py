...
logger = logging.getLogger("depth_anything3.dinov2")
...
class DinoVisionTransformer(nn.Module):
    ...
    def prepare_tokens_with_masks(self, x, masks=None, cls_token=None, **kwargs):
        ...
        x = x.reshape(B, S, x.shape[-2], x.shape[-1])
        return x
    ...
    def _prepare_rope(self, B, S, H, W, device):
        ...
        if self.rope is not None:
            ...
            pos = pos.reshape(B, S, pos.shape[-2], pos.shape[-1])
            ...
            if self.patch_start_idx > 0:
                ...
                pos_special = pos_special.reshape(B, S, pos_special.shape[-2], pos_special.shape[-1])
