"""TinyViT encoder used by MobileSAM.

The module keeps upstream MobileSAM/TinyViT attribute names so converted
checkpoints load with ``strict=True``. See ``NOTICE`` for third-party
...
class DropPath(nn.Module):
    ...
    def forward(self, x):
        return drop_path(x, float(self.drop_prob or 0.0), self.training)

    def __repr__(self):
        return f"{type(self).__name__}(drop_prob={self.drop_prob})"
