...
def parse_nested_data_json_label(json_path: Path, img_w, img_h):
    ...
    rect is treated as [x1, y1, x2, y2] (absolute pixel coords) - the standard convention
    for this schema - with an automatic fallback to [x, y, w, h] only if the xyxy reading
    would be an invalid (non-positive) box.
    ...
    for cls_name, obj_list in data.items():
        ...
        for obj in obj_list:
            for raw_rect, _region_type in _find_rects_in_obj(obj):
                x1, y1, x2, y2 = [float(v) for v in raw_rect]
                if x2 <= x1 or y2 <= y1:
                    # not a valid xyxy box - fall back to xywh interpretation
