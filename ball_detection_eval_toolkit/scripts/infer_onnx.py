#!/usr/bin/env python3
"""
Run ONNX inference on a single image or directory of images, drawing
predicted ball boxes (yellow) for visual sanity-checking.

Ground truth is OPTIONAL here (inference doesn't need labels) -- but if you
pass annotation-format arguments, matching GT boxes (green) are drawn
alongside predictions so you can visually compare, in any of the three
supported formats: client / coco / yolo.

Examples:

    # Pure inference, no GT overlay
    python scripts/infer_onnx.py --onnx model.onnx --input path/to/img_or_dir \
        --out-dir vis_output --input-w 480 --input-h 320

    # With client-format GT overlay
    python scripts/infer_onnx.py --onnx model.onnx --input /path/to/basic_eval_caseA \
...
def parse_args():
    ...
    p.add_argument("--input", required=True, help="Image file or directory of images to run inference on")
    ...
    p.add_argument("--score-thresh", type=float, default=0.3,
                   help="Visualization-only threshold (raise for cleaner display)")
    p.add_argument("--nms-iou", type=float, default=0.5)

    p.add_argument("--ann-format", default=None, choices=[None, "client", "coco", "yolo"],
                   help="Optional: overlay GT boxes from this annotation format for visual comparison")
    # dataset args are only required if --ann-format is given; make them optional here
...
def build_gt_lookup(args):
    """Returns {image_path: (N,4) xyxy GT boxes} or {} if no --ann-format given."""
...
def main():
    ...
    detector = ONNXBallDetector(
        args.onnx, input_size=(args.input_w, args.input_h),
    ...
    for img_path in image_paths:
        ...
        print(f"{img_path.name}: {len(boxes)} ball(s) detected"
              f"{f', {len(gt_boxes)} GT box(es)' if args.ann_format else ''} -> {out_path}")
