#!/usr/bin/env python3
"""
Run PicoDet inference on a single image or directory, drawing predicted
ball boxes for visual sanity-checking. Optional GT overlay in any of the
three annotation formats, same as infer_onnx.py.

*** REQUIRES models/picodet_backend.py to be completed first ***

Example:
    python scripts/infer_picodet.py --model-dir path/to/exported_picodet \
...
def main():
    ...
    detector = PicoDetBallDetector(
        ...
        score_thresh=args.score_thresh, nms_iou_thresh=args.nms_iou, use_gpu=args.use_gpu,
    )
